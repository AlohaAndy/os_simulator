/**
  * Driver program parse structures
  * Allows for parsing of the ConfigFile structure
...
int readMetaDataLine(MetaDataNode* head, char* line);
int RemoveSpaces(char* string);


int readConfig(ConfigFile* configFile, char* pathToFile)
{
...
int readMetaData(MetaDataNode* head, char* pathToFile)
{
    // Attempt to open file path
    ...
    while (fgets(line, sizeof(line), metaFP))
    {
        ...
        {
            int lineError = readMetaDataLine(head, line);
